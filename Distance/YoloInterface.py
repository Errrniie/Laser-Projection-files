...
def _display_worker():
    ...
    while not _stop_event.is_set():
        try:
            frame, bbox, conf, feet_center = _display_queue.get(timeout=0.1)
            vis = frame.copy()
...
def show_frame(frame, bbox=None, conf=None, feet_center=None):
    """Non-blocking frame display."""
    try:
        _display_queue.put_nowait((frame, bbox, conf, feet_center))
    except queue.Full:
        pass
